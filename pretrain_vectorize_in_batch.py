...
df = pd.read_excel("data/short_video_with_tags-en-0607.xlsx")

start_time = time.time()
with open("results/short_video_text_vector_en.txt", "w", encoding="UTF-8") as f:
    for i in range(len(df)):
        id = df["id_x"][i]
        sent = df["video_text"][i]
        array = list(sent2vec(sent))
        f.write(str(id))
        for a in array:
            f.write("\t" + str(a))
        f.write("\n")
